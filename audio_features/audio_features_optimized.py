...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', required=True, help='Path to audio file')
    parser.add_argument('--features', nargs='+', required=True,
                        help='List of features to extract (e.g. mean_f0 articulation_rate)')
    parser.add_argument('--start', type=float, default=0.0, help='Segment start time in seconds')
    parser.add_argument('--end', type=float, help='Segment end time in seconds (default: full duration)')
    parser.add_argument('--verbose', action='store_true', help='Print feature values')
    args = parser.parse_args()

    extractor = AudioFeaturesOptimized(path=args.path)
    end_time = args.end if args.end is not None else extractor.sound.get_total_duration()
    stats = extractor.extract_segment(args.features, args.start, end_time, args.verbose)
    import json
